...
export function AgentWalletModal({ open, onOpenChange }: AgentWalletModalProps) {
  ...
  const [tokenBalances, setTokenBalances] = useState<{ xtz: string } | null>(null)
  ...
  const displayBalance = tokenBalances?.xtz || "0.00"
      ...
      >
        {hasWallet ? (
          <>
            ...
            <span className="font-semibold">{displayBalance} XTZ</span>
          </>
        ) : (
          <>
