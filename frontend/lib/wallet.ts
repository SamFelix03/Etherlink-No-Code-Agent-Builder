...
/**
 * Fetch XTZ token balance (native token of Etherlink Shadownet)
 * @param address - The wallet address
 * @returns XTZ token balance as string
 */
export async function getTokenBalances(address: string): Promise<{
  xtz: string
}> {
  const ETHERLINK_RPC_URL = 'https://node.shadownet.etherlink.com'

  try {
    const provider = new ethers.JsonRpcProvider(ETHERLINK_RPC_URL)
    
    // Get native XTZ balance (native token, not ERC-20)
    const balance = await provider.getBalance(address)

    // Format balance (XTZ uses 18 decimals like ETH)
    const formattedBalance = ethers.formatEther(balance)
    const numericBalance = parseFloat(formattedBalance)

    // Format to 2 decimal places
    const xtzBalance = numericBalance.toFixed(2)
